...
class ChatbotController {
  ...
  static async sendResponse(req, res) {
    try {
      ...
      const result = await ChatbotService.generateResponse(sessionId, message);
      ...
      return res.status(500).json({
        ...
        fallbackResponse:
          "I'm sorry, I'm experiencing technical difficulties. Please try again later. If you're in crisis, please contact emergency services or a mental health helpline.",
      });
    }
  }
  ...
  static async getResponse(req, res) {
    try {
      ...
      const result = ChatbotService.getConversationHistory(sessionId);
  ...
  static async clearChat(req, res) {
    try {
      ...
      const result = ChatbotService.clearConversation(sessionId);
